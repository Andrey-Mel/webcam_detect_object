        ...
        while True:
            ...
            if not queue1.empty():
                ...
                if cam_id not in writers:
                    filename1 = fr"record_cam1\camera_{cam_id}_{current_time}.mp4"
                    fourcc = cv2.VideoWriter_fourcc(*'MP4V') # or 'MP4V' for mp4
                    writers[cam_id] = cv2.VideoWriter(filename1, fourcc, 23.0, (frame1.shape[1], frame1.shape[0]))
            ...
            if not queue2.empty():
                ...
                if cam_id not in writers:
                    ...
                    writers[cam_id] = cv2.VideoWriter(filename2, fourcc, 23.0, (frame2.shape[1], frame2.shape[0]))
                    print(f'Record start from cam - {cam_id}')
